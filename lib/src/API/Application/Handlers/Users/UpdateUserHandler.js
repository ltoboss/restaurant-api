...
class UpdateUserHandler {
    ...
    handle(command) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const userExist = yield this.userServices.getById(user.id);
            const emailExist = yield this.userServices.getByEmail(user.email);
            if (emailExist && userExist.id !== emailExist.id) {
                throw new InvalidArgumentException_1.default(`The email ${user.email} is already taken`);
            }
            if (command.getPassword()) {
                const cryptoHashPasswordServices = new CryptoHashPasswordServices_1.default();
